...
export async function verifyStudent(req, res, next) {
    try {
        ...
        let exist = await StudentModel.findOne({ username });
...
export async function checkone(req, res, next) {
    try {
        ...
        let usernameExist = await StudentModel.findOne({ username });
        let emailExist = await StudentModel.findOne({ email });
...
/** GET: http://localhost:8080/api/student/example123 */
...
export async function updateUser(req, res) {
    try {
        ...
        if (userId) {
            ...
            // update the data
            StudentModel.updateOne({ _id: userId }, body, function (err, data) {
                if (err) throw err;
        ...
        try {
            ...
            StudentModel.findOne({ username })
                .then(user => {
                    bcrypt.hash(password, 10)
                        .then(hashedPassword => {
                            StudentModel.updateOne({ username: user.username },
